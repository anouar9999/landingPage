...
const PassGamers = () => {
  ...
  // Vérifier la taille d'écran pour adaptation mobile
  useEffect(() => {
    const checkMobile = () => {
  ...
  // Animation de la carte - optimisée pour mobile
  useEffect(() => {
    if (cardRef.current) {
      ...
      // Animation des éléments holographiques - seulement sur desktop
      if (!isMobile) {
      ...
      // Animation 3D au survol - seulement sur desktop
      if (!isMobile) {
    ...
    // Animation d'entrée des avantages - adapté pour mobile
  ...
  return (
    ...
    >
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div className="flex flex-col lg:flex-row items-center gap-8 md:gap-12">
          ...
          <div className="lg:w-1/2 text-center lg:text-left">
            ...
            <h2 className="text-4xl md:text-5xl text-yellow-500 mb-4 md:mb-6 font-nightWarrior">
              {t('passGamers.title', "Pass Gamer Officiel")}
            </h2>
            
            <p className={`text-white/80 text-lg md:text-xl mb-4 md:mb-6 ${getTextClass()}`}>
            ...
            <div className="inline-block bg-green-500/20 px-4 py-2 rounded-lg mb-6 md:mb-8">
              <span className={`text-green-400 font-bold ${getTextClass()}`}>
                {t('passGamers.freeOfCharge', "100% Gratuit - Service Public")}
              </span>
            </div>
            
            {/* Carte du Pass animée - redimensionnée pour mobile */}
            <div className="max-w-xs mx-auto lg:mx-0">
              <div 
                ref={cardRef} 
                className={`card-3d-container w-full ${isMobile ? 'h-40 sm:h-48' : 'h-56'} mt-6 md:mt-8 perspective-1000 cursor-pointer transform hover:scale-105 transition-transform duration-300`}
              ...
              >
                <div className="card-3d relative w-full h-full rounded-xl bg-gradient-to-br from-[#1a1c2e] to-[#121320] shadow-2xl overflow-hidden border border-yellow-500/30">
                  ...
                  {/* Texture holographique - masquée sur mobile pour meilleures performances */}
                  <div className={`absolute inset-0 opacity-10 bg-grid-pattern ${isMobile ? 'hidden' : ''}`}></div>
                  
                  {/* Contenu de la carte - adapté pour mobile */}
                  <div className="p-4 sm:p-6 h-full flex flex-col justify-between">
                    <div className="flex justify-between items-start">
                      <div>
                        ...
                        <div className="text-base sm:text-lg font-bold text-white">Pass Gamer Officiel</div>
                      </div>
                      <div className="w-8 h-8 sm:w-12 sm:h-12 rounded-full bg-gradient-to-br from-yellow-500 to-yellow-600 flex items-center justify-center">
                        <Award size={isMobile ? 16 : 24} className="text-black" />
                      </div>
                    </div>
                    ...
                    <div className="mt-auto">
                      ...
                      <div className="flex justify-between items-center">
                        <div>
                          ...
                          <div className="text-xs sm:text-sm text-white">Permanente</div>
                        </div>
                  ...
                  {/* Éléments holographiques - masqués sur mobile pour meilleures performances */}
                  {!isMobile && (
                    <>
            ...
            <div className="mt-6 md:mt-8 flex flex-col sm:flex-row gap-3 md:gap-4 justify-center lg:justify-start">
              <a 
                href="#inscription" 
                className="group inline-flex items-center justify-center gap-2 bg-yellow-500 hover:bg-yellow-600 text-black font-bold py-2 md:py-3 px-4 md:px-6 rounded-lg transition-all duration-300"
              >
                <span>{t('passGamers.getYourPass', "Obtenir votre Pass")}</span>
                <ChevronRight size={isMobile ? 16 : 18} className="transition-transform group-hover:translate-x-1" />
              </a>
          ...
          {/* Colonne de droite: Avantages - responsive pour mobile */}
          <div className="lg:w-1/2 mt-12 lg:mt-0">
            <h3 className="text-white text-xl md:text-2xl font-bold text-center lg:text-left mb-4 md:mb-6">
              {t('passGamers.advantages', "Avantages du Pass Gamer")}
            </h3>
            
            <div className="benefits-grid grid grid-cols-1 sm:grid-cols-2 gap-3 md:gap-4">
              {passAdvantages.map((advantage, index) => (
                <div 
                  key={index}
                  className="benefit-card bg-black/30 backdrop-blur-sm rounded-lg border border-yellow-500/20 p-3 md:p-4 hover:border-yellow-500/40 transition-all duration-300"
                >
                  <div className="flex items-center gap-2 md:gap-3 mb-2">
                    <div className="flex-shrink-0 w-8 h-8 md:w-10 md:h-10 rounded-lg bg-yellow-500/10 flex items-center justify-center text-yellow-500">
                      {React.cloneElement(advantage.icon, { size: isMobile ? 16 : 24 })}
                    </div>
                    <h3 className="text-white text-base md:text-lg font-bold">{advantage.title}</h3>
                  </div>
                  <p className="text-white/70 text-xs md:text-sm ml-10 md:ml-13">{advantage.description}</p>
                </div>
              ))}
            </div>
            
            {/* Comment obtenir le Pass - Version compacte adaptée pour mobile */}
            <div className="mt-6 md:mt-8 bg-black/40 backdrop-blur-sm rounded-xl border border-yellow-500/20 p-4 md:p-5">
              <h3 className="text-white text-lg md:text-xl mb-3 md:mb-4 font-bold">{t('passGamers.howToGet', "Comment obtenir votre Pass")}</h3>
              <div className={`flex items-center ${isMobile ? 'flex-col space-y-4' : 'justify-between'}`}>
                <div className={`flex ${isMobile ? 'flex-col space-y-4' : 'gap-4'}`}>
      ...
      {/* Emplacement publicitaire intégré au contenu - rendu responsive */}
      <div className="max-w-7xl mx-auto px-4 mt-12 md:mt-16 mb-8 md:mb-10">
